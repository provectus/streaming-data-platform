...
public class ParquetFileReader implements Closeable {
  ...
  public static <T> List<T> runAllInParallel(int parallelism, List<Callable<T>> toRun) throws ExecutionException {
  ...
  /* -------------------------------- Cinarra additional methods start --------------------------------------- */
  ...
  public Optional<PageReader> readColumnInBlock(int blockIndex, ColumnDescriptor columnDescriptor) throws IOException {
    ...
    if (mc.isPresent()) {
      ColumnChunkMetaData column = mc.get();
      ChunkDescriptor chunk = new ChunkDescriptor(columnDescriptor, column, column.getStartingPos(), (int) column.getTotalSize());
      return Optional.of(readChunk(f, chunk));
    }
    return Optional.empty();
  }

  private ColumnChunkPageReader readChunk(SeekableInputStream f, ChunkDescriptor descriptor) throws IOException {
    f.seek(descriptor.fileOffset);

    int fullAllocations = descriptor.size / options.getMaxAllocationSize();
    int lastAllocationSize = descriptor.size % options.getMaxAllocationSize();
  ...
  /* -------------------------------- Cinarra additional methods end ---------------------------------------- */
}
