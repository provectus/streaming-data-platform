...
for i in `seq 1 $1`
	do
	txid=$RANDOM
	aws apigateway test-invoke-method --rest-api-id=${REST_ID} --http-method=GET --resource-id=${bid_resource} --path-with-query-string /bids?appuid=123\&txid=${txid}\&campaign_item_id=${CID}
